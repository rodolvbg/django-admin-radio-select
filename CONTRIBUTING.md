...
- `tests/` — pytest tests (`test_package.py`, `test_widgets.py`, `test_mixins.py`, `test_formsets.py`, `test_admin_integration.py`) and vitest tests (`tests/js/`).
...
Visit `http://127.0.0.1:8000/admin/`. `Album` uses a `TabularInline` for its images; `Album (stacked inline demo)` (a proxy model) uses a `StackedInline` — so both render paths can be exercised by hand.
